...
def find_form(board,  word, y, x, seen):
    "find word in board, starting at x, y"""
    #recursivley finding smaller words

    # base case: not the correct letter
    ...
    if y < 4:
        if find_form(board, word[1:], y + 1, x, seen):
            return True

    if x > 0:
        if find_form(board, word[1:], y, x + 1, seen):
            return True
...
def find(board, word):
    ...
    print("%-6s%d,%d    %-3s%-8s%-30s" %("out", "y", "x", "bd", "word" "seen"))

    #try slots on board to find letter
