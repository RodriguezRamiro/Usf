...
function find(board, word, y, x, seen) {
  /** Can word be found in board? */

  // TODO
  // base case not right letter

  if (board[y][x] != word[0]) return false;

  //Base case: already used letter

  if(seen.has(y+ "-" + x)) return false;

  //Base case: down to the last letter
  ...
  if (x > 0 && findFrom(board, word.slice(1), y - 1, x, seen )) return true;
  if (x < 4 && findFrom(board, word.slice(1), y - 1, x, seen )) return true;

  //cant find next letter, path dead

  return false;
}
function find(board, word){
  /** is word found in borad */
  //find starting letter
  // win

  for(let y = 0; y < 5; y++)
    for (letx = 0; x < 5; x++)
    if (findFrom(board, ord, y, x, new Set())) return true;
