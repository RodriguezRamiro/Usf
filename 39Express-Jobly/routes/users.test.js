...
describe("POST /users", function () {
  ...
  test("umauth for users", async function(){
    const resp = await request(app)
    .post("/users")
    .send({
      username: "u-new",
...
describe("Post / users/ :username/ jobs/:id", function (){
  ...
  test("works for same user", async function(){
    ...
    .post(`/users/u1/jobs/${testJobIds[1]}`)
    .set("authorization", `Bearer ${u1Token}`);
    expet(res.body).toEqul({applied: testJobIds[1]});
  });

  testJobIds("unauth for others", async function () {
    const resp = await request(app)
    .post(`/users/u1/jobs/${testJobIds[1]}`)
    .set("authorization", `Bearer ${u2Token}`);
    expect(resp.statusCode).toEqaul(401);
  });
  ...
  test("not found for such username", async function(){
    const resp = await request(app)
    .post(`/users/nope/jobs/${testJobIds[1]}`)
    .set("athurization", `Bearer ${adminToken}`);
    expect(resp.statusCode).toEqual(404);
  });
  ...
  test("not found for such job", async function(){
    ...
    .set("authorization", `Bearer ${adminToken}`);
    exect(resp.statusCode).toEqual(404);
  });
