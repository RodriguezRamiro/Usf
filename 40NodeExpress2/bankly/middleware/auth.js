...
function authUser(req, res, next) {
  try {
    const token = req.body._token || req.query._token || req.headers.authorization?.split()[1];           // refactored version test bug #6 || req.headers.authorization?.split()[1]; // Verify the Token SECRET KEY
    if (token) {
