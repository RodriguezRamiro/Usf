...
// Registration TEST CASE! 1
//*************************Ensure comprehensive access control for the bankly application: *******************************************************
test("should not allow registration with missing fields", async function() {
  const response = await request(app)
    .post("/auth/register")
    .send({
      username: "incomplete_user",
      // Missing password
      first_name: "incomplete_first",
      last_name: "incomplete_last",
...
test("should not allow registration with weak password", async function() {
  const response = await request(app)
    .post("/auth/register")
    .send({
  ...
  expect(response.statusCode).toBe(400);
  expect(response.body.message).toContain("Password too weak");
});

test("should not allow registration with invalid email format", async function() {
  const response = await request(app)
    .post("/auth/register")
    .send({
...
// Login Edge Cases 2
//*************************Ensure comprehensive access control for the bankly application: *******************************************************
test("should not allow login with incorrect password", async function() {
  const response = await request(app)
    .post("/auth/login")
    .send({
      username: "u1",
...
test("should not allow login with unregistered username", async function() {
  const response = await request(app)
    .post("/auth/login")
    .send({
...
//Retrieval Edge Cases 3
//*************************Ensure comprehensive access control for the bankly application: *******************************************************

test("should return 404 if user does not exist", async function() {
  const response = await request(app)
...
test("should return 403 if trying to access another user's data without permission", async function() {
  const response = await request(app)
    .get("/users/u2")
    .send({ _token: tokens.u1 }); // u1 should not access u2's data
...
test("should return 400 if trying to update with invalid data", async function() {
  const response = await request(app)
    .patch("/users/u1")
    .send({ _token: tokens.u3, email: "invalidemail" }); // Invalid email
  expect(response.statusCode).toBe(400);
  expect(response.body.message).toContain("Invalid email format");
});

test("should return 403 if trying to update another user's data without permission", async function() {
  const response = await request(app)
    .patch("/users/u2")
    .send({ _token: tokens.u1, first_name: "new_name" }); // u1 should not update u2's data
  expect(response.statusCode).toBe(403);
  expect(response.body.message).toBe("Forbidden");
});
...
//Deletion Edge Cases 5
//*************************Ensure comprehensive access control for the bankly application: *******************************************************

test("should return 404 if trying to delete a non-existent user", async function() {
  const response = await request(app)
    .delete("/users/nonexistent_user")
    .send({ _token: tokens.u3 });
  expect(response.statusCode).toBe(404);
  expect(response.body.message).toBe("User not found");
});

test("should return 403 if trying to delete another user without admin privileges", async function() {
  const response = await request(app)
    .delete("/users/u2")
    .send({ _token: tokens.u1 }); // u1 should not delete u2
  expect(response.statusCode).toBe(403);
  expect(response.body.message).toBe("Forbidden");
});
